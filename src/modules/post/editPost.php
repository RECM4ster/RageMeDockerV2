<?php
include('../login/checkCookie.php');
include ('../post/getPost.php');
...
$getPost = new getPost($_GET['postId']);
$data = $getPost->getPostData();
$viewData = mysqli_fetch_array($data);
?>
...
<form id="loginForm" action="updatePost.php" METHOD="post">
    <label class="loginLabel loginElement">Tytuł</label></br>
    <input type="text"  class="loginInput loginElement" name="title" value="<?php echo $viewData['pos_topic']?>"></br>
    <input type="hidden" name = "id" value="<?php echo $viewData['pos_id']?>"></br>
    <label class="loginLabel loginElement">Treść</label></br>
    <textarea class="loginInput loginElement" name="text" style="height:300px"><?php echo $viewData['pos_body']?></textarea></br>
